...
FSkeletonMesh * FSkeletonMesh::New(const std::string & ModelFilePath)
{
	FSkeletonMesh* SkeletonMesh = new FSkeletonMesh();
	SkeletonMesh->ModelFilePath = ModelFilePath;
	SkeletonMesh->Directory = ks::File::getParentDir(ModelFilePath);
	SkeletonMesh->LoadModel();

	if (SkeletonMesh->HasAnimations())
	{
		SkeletonMesh->SelectedAniamtionIndex = 0;
	}

	for (int i = 0; i < SkeletonMesh->Scene->mNumAnimations; i++)
	{
		aiAnimation* Animation = SkeletonMesh->Scene->mAnimations[i];
		SkeletonMesh->Animations.push_back(Animation);
	}

	SkeletonMesh->ProcessNode(SkeletonMesh->Scene->mRootNode);
	return SkeletonMesh;
}
